...
OPENCL_DOCS_RUNTIME_LAYER_ASCIIDOC_URL = "https://raw.githubusercontent.com/KhronosGroup/OpenCL-Docs/refs/heads/main/api/opencl_runtime_layer.asciidoc"
OPENCL_DOCS_XML_URL = "https://raw.githubusercontent.com/KhronosGroup/OpenCL-Docs/refs/heads/main/xml/cl.xml"
...
def string_from_url(url: str) -> str:
...
def generate_bindings(dstdir: str, go_prelude: str, headers: list[str], asciidocs: list[str], cl_xml: str, custom_bindings: CustomBindings) -> None:
    with open(os.path.join(dstdir, "doc.go"), "w") as o:
        o.write("// Package cl contains OpenCL functions.\npackage cl")

    func_details: dict[str,FunctionDetails] = {}
    for asciidoc in asciidocs:
        with open(asciidoc) as f:
            s = f.read()
            for m in re.finditer(r"^\[open,refpage='(?P<name>[^']*)',desc='(?P<desc>[^']*)',type='(?P<typ>[^']*)'(,alias='(?P<alias>[^']*)')?\]$",
                s, flags=re.MULTILINE):
                func_details[m["name"]] = FunctionDetails(description=m["desc"])

    func_deprecated_in_version: dict[str, str] = {}
    with open(cl_xml, encoding="utf-8") as f:
        s = f.read()
        for m in re.finditer(r'<require comment="[\w \.]+ deprecated in OpenCL (?P<deprecated_ver>[\d\.]+)">(?P<body>(\n.*?)+)</require>', s, flags=re.MULTILINE):
            for n in re.finditer(r'<\w+ name="(?P<name>.*)"\/>', m["body"], flags=re.MULTILINE):
                func_deprecated_in_version[n["name"]] = m["deprecated_ver"]
            ...
            if os.path.basename(header) == "cl.h":
                ...
                for enum in enums:
                    if enum.ctyp in ("cl_version", "cl_bool"):
                        continue
                    is_bitfield = enum.ctyp in typedef_typs and typedef_typs[enum.ctyp] == "cl_bitfield"
                    o.write(f"// Returns a string representation of the {enum.gotyp}")
                    if is_bitfield:
                        o.write(f" in form of each known flag name separated by |.\n//\n// If any unmatched bits are contained, they are added as one \"UNKNOWN(0x...)\" pseudo-flag.\n")
                    else:
                        o.write(f" in form of the enumeration value name, or \"UNKNOWN (123)\" if enumeration value is unknown.\n")
                ...
                for func in funcs:
                    ...
                    if func.name in func_details:
                        o.write(f"// Description: {func_details[func.name].description}\n")
                        o.write("//\n")
                    o.write(f"// Introduced in OpenCL version {api_version(func.api_version)}.\n")
                    o.write("//\n")
                    if api_version_deprecated(func.api_version):
                        o.write(f"// Deprecated: Deprecated by OpenCL version {func_deprecated_in_version[func.name]}.\n")
                        o.write("//\n")
                    o.write(f"// See {docs_url(func.name)}.\n")
...
if __name__ == "__main__":
    ...
    if not os.path.isdir("opencl-docs"):
        pathlib.Path("opencl-docs").mkdir(parents=True, exist_ok=True)
        with open("opencl-docs/opencl_runtime_layer.asciidoc", "w", encoding="utf-8") as f:
            f.write(string_from_url(OPENCL_DOCS_RUNTIME_LAYER_ASCIIDOC_URL))
        with open("opencl-docs/cl.xml", "w", encoding="utf-8") as f:
            f.write(string_from_url(OPENCL_DOCS_XML_URL))
    ...
    generate_bindings(
        ...
        ["opencl-docs/opencl_runtime_layer.asciidoc"],
        "opencl-docs/cl.xml",
